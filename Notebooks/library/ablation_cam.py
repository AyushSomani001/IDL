...
from find_layers import replace_layer_recursive
...
The process involves ablating individual activations and measuring the corresponding decrease in the target score. 

The target layer activations are cached, while previous layers are not cached. 
This setup saves time when the target layer is a large block, such as "model.features" in VGG. 
Ablating many channels requires a forward pass, which can be time-consuming. 

To speed things up, channels that don't contribute can be skipped using the 
"ratio_channels_to_ablate" parameter. The default value of 1.0 means that all 
channels are ablated, but this experimental method can be improved.
"""
